...
<br><br><br><br><br>
<div class = "container">     
	<h2 align="center">Λογαριασμοί αποφοίτων που γραφτήκαν<br>την ίδια χρονιά με εμένα</h2><hr>	
...
if(sizeof($result)>0) {
	...
	foreach($result as $row) {
		...
		if($registration_year == 0)
		{
			echo '<tr><td colspan = "6" class="text-center">Θα πρέπει να συμπληρώσετε το έτος εισαγωγής στην φόρμα με τα προσωπικά σας δεδομένα</td></tr>';			
		...
		else
		{
			...
			$stmt2 = $conn->prepare("SELECT * FROM users WHERE registration_year = ? AND id != ?");
			$stmt2->execute(array($registration_year, $alumni_id));
