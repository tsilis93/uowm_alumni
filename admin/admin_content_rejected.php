...
<br><br><br><br>
<h2>Μη Δημοσιευμένες Ανακοινώσεις</h2>
...
$stmt = $conn->prepare("SELECT * FROM contents WHERE status = 0 AND publication_date IS NOT NULL");
$stmt->execute();
...
if(sizeof($result)>0) {
	foreach($result as $row)
	{
		...
		echo	'<td width = "20%">'; 
		echo    '<form action="content_overview.php?rejected_content='.$id.'" method="post">';
...
else
{
		...
		echo    '<tr><td colspan = "5"> Δεν υπάρχουν μη δημοσιευμένες ιστορίες</td></tr>';
}
