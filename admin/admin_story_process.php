...
$stmt2 = $conn->prepare("SELECT * FROM stories WHERE status = 1");
$stmt2->execute();
...
$stmt2 = $conn->prepare("SELECT * FROM stories WHERE status = 1 ORDER BY publication_date desc LIMIT $from, $perPage");
$stmt2->execute();
$result = $stmt2->fetchAll();
...
if(sizeof($result)>0) {
	foreach($result as $row)
	{
		...
		echo	'<td width = "20%">'; 
		echo    '<form action="story_overview.php?published_story='.$id.'" method="post">';
...
else
{
		...
		echo    '<tr><td colspan = "5"> Δεν υπάρχουν ιστορίες που έχουν δημοσιευτεί </td></tr>';
}
