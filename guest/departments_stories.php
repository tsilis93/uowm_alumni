...
{			
	echo '<tr><td colspan = "5"> Δεν υπάρχουν άλλες διαθέσιμες Ιστορίες </td></tr>';
}
...
?>
	...
	κουμπί για το φερε περισσότερα δεδομένα
	
	<div class = "show_more">
		 <input class="btn btn-link" type="button" id = "btnMore" value = "Δείτε και άλλες Ιστορίες" /> 
	</div>
...
/*	Η συνάρτηση $(window).scroll(function() { δεν λειτουργεί στα κινητα οπότε την αντικαθηστώ με την $(document.body).on('touchmove', onScroll); // for mobile
	
	$(window).scroll(function() {   
        if($(window).scrollTop() + $(window).height() >= $(document).height()) {
            var last_id = $(".story-id:last").attr("id");
...
*/
	...
	function loadMoreData(last_id){
		var rowCount = $('#cssTable tr').length; //μετράω τον αριθμό των ιστοριών στον πινακα,
		rowCount = ((rowCount-1)/2)+1; //αφαιρώ την 1η γραμμή που ειναι η επικεφαλίδα, διαιρώ με το 2 για να βγάλω τις 
											// γραμμές των περιεχομένων και προσθέτω 1 για την επόμενη ιστορία.
		$.get("more_departments_stories.php", {last_id: last_id, id: id, row_count2: rowCount}, function(data, status) {
	...
	/*  Φόρτωση περισσότερων δεδομένων με button 
	
	$('#btnMore').click(function() {

		var last_id = $(".story-id:last").attr("id");
		
		$.get("more_departments_stories.php", {last_id: last_id, id: id}, function(data, status) {
