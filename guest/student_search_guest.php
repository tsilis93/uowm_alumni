...
<script>
	$(document).ready(function() {
		...
		var did = <?php echo(json_encode($id)); ?>;
		...
		$('#btnSearchAll').click(function() {  // επιστρέφει όλους τους απόφοιτους από όλα τα τμήματα
			var all = true;
			//var departid = <?php echo(json_encode($id )); ?>;
			$.post("search.php", {
				All: all,
				departmentId: did
			}, function(data, status) {
...
function mouseOver()
{
	var dname = <?php if(!empty($dname)) { echo(json_encode($dname)); } else { echo(json_encode("")); }?>;
	$('#info').html('Μπορείς να κάνεις αναζήτηση αποφοίτων για το ' + dname);
	$('#info').show();
}
function mouseOut() 
{
