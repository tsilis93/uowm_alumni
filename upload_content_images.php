...
foreach($imagesTOupload as $name) { 
	...
	$stmt = $conn->prepare("INSERT INTO images (images_path, contentID, departmentID, userID, storyID) VALUES (?, ?, ?, ?, ?)");
	$stmt->bindParam(1, $name);
	$stmt->bindParam(2, $id);
	$stmt->bindParam(3, $departmentID);
	$stmt->bindParam(4, $userID);
	$stmt->bindParam(5, $storyID);
	if ($stmt->execute()) {
	}
}
...
$text = "Προστέθηκαν " .$counter." εικόνες στην δημοσίευση με τίτλο '" . $title . "' από τον χρήστη " . $user . "";
$admin = 1;
$alumni = 0;
...
header("Location: alumni/content_overview.php?content=$id");
